...
class Field:
    ...
    A Field will be validated based on its primitive type and (when
    provided) all its validators (for more information about how Validators
    work, check the Validator's documentation).
    """
    ...
    def __init__(self,
                 ...
                 dict_name: str = None):
        self.type = type
        # when using items_config, each element of the list becomes a
        # config object.
        ...
        if items_config and type != list:
            raise RuntimeError("items_config can be used only with a Field"
                               " of type list.")
...
class Config(ABC):
    ...
    To create a new Config class, extend this base and create the config
    spec using the Field object as follows:
    ...
    When creating the Config object, fields values must be passed in as
    keyword arguments. Accessing the values of the config objects is done
    using the dot notation, just like any other object. E.g.
    ...
    def __new__(cls, *args, **kwargs):
        ...
        their value (or default value) and the original Field objects are
        saved under the `_fields` class field.
        ...
        """
        if args:
            raise RuntimeError("Cannot provide positional arguments to Config"
                               " class.")
    ...
    def __init__(self, **kwargs):
        ...
        for input_var in kwargs.keys():
            if input_var not in self._fields.keys():
                raise RuntimeError(
                    "%s: '%s' was provided but the config spec does not"
                    " contain any field with that name."
                    % (self._get_exception_msg_prefix(), input_var))

        for name, field_obj in self._fields.items():
            if field_obj.default_value is not None and field_obj.required:
                log.warning("The 'required' flag for field '%s' is being"
                            " ignored since a default value was provided."
                            % name)
            input_value = kwargs.get(name, field_obj.default_value)
    ...
    def to_dict(self) -> Dict[str, any]:
        ...
        for field_name, field_obj in self._fields.items():
            ...
            elif field_obj.items_config:  # the field is a list of Configs
