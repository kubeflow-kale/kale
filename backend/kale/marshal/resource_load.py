...
def resource_all(uri, *args, **kwargs):
    ...
    >>> resource_load('myfile.csv')  # doctest: +SKIP
    <odo.CSV ...>

    Database connection strings may embed connection information

    >>> resource_load('postgresql://user:pass@hostname/db::tablename')  # doctest: +SKIP
    ...
    >>> resource_load.funcs  # doctest: +SKIP
    ...
    >>> odo(resource_load('some-sorce'), target)  # doctest: +SKIP
    ...
    >>> resource_load('myfile.hdf5::/data', dshape='1000 * 1000 * float32')  # doctest: +SKIP
