...
class Compiler:
    ...
    def _get_templating_env(self, templates_path=None):
        ...
        # pipeline parameters helpers
        template_env.filters['pps_names'] = lambda pps: sorted(pps)
        template_env.filters['pps_types'] = lambda pps: [pps[k].param_type
                                                         for k in sorted(pps)]
        template_env.filters['pps_values'] = lambda pps: [pps[k].param_value
                                                          for k in sorted(pps)]
