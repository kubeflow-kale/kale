...
export const InlineMetadata: React.FC<IProps> = ({
  ...
  const isMergedCell = !blockName;
  const isReserved = RESERVED_CELL_NAMES.includes(blockName);
  const cellTypeClass = isReserved ? 'kale-reserved-cell' : '';
  const name = blockName || previousBlockName;
  ...
  return (
    <div ref={wrapperRef} className={'kale-inline-cell-metadata-container'}>
      ...
      >
        ...
        <Tooltip
          placement="top"
          key={blockName + 'tooltip'}
          title={
            isReserved
              ? RESERVED_CELL_NAMES_HELP_TEXT[blockName]
              : 'This cell starts the pipeline step: ' + blockName
          }
        >
          <Chip
