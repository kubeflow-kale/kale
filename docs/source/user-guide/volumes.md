...
## ReadWriteOnce Warning

When you compile or run a pipeline with **ReadWriteOnce (RWO)** volumes, Kale
...
RWO volumes can only be attached to pods running on the **same Kubernetes
node**. If your pipeline has parallel steps (steps that don't depend on each
other), Kubernetes may schedule them on different nodes. When this happens,
some steps will fail because they can't mount the RWO volume.

### Solutions

1. **Use ReadWriteMany (RWX) volumes** — These can be mounted by pods on any
   node. Most network file systems (NFS, CephFS, etc.) support RWX.

2. **Ensure sequential execution** — If all steps depend on each other (no
   parallelism), they'll run one at a time and the RWO limitation won't apply.
